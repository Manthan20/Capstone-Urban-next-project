  ...
  useEffect(() => {
    const fetchProperties = async () => {
      try {
        ...
        // Filter out sold properties and limit to first 6 non-sold properties
        const availableProperties = response.data.filter((property) => !property.sold).slice(0, 6);
        setProperties(availableProperties); // Set the properties state
      } catch (error) {
